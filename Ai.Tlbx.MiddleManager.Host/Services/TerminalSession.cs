...
public sealed class TerminalSession : IDisposable
{
    ...
    public async Task SendInputAsync(ReadOnlyMemory<byte> data)
    {
        ...
        try
        {
            Log.WriteInput($"Session {Id}: Writing {data.Length} bytes to PTY input");
            ...
            Log.WriteInput($"Session {Id}: Input write+flush complete");
        }
        catch (Exception ex)
        {
            Log.WriteInput($"Session {Id}: Input write failed: {ex.Message}");
        }
    }
